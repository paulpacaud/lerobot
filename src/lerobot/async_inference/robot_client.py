...
class RobotClient:
    ...
    def __init__(self, config: RobotClientConfig):
        ...
        if config.action_space == "ee":
            ...
            # initial_guess_current_joints=True in the IK processor causes the solver to find different arm configurations on each frame leading to dangerous erratic behavior. Changed to False so it uses the previous IK solution as the starting point, maintaining trajectory continuity.
            self.ik_processor = RobotProcessorPipeline(
                steps=[
                    InverseKinematicsEEToJoints(
                        kinematics=kinematics,
                        motor_names=motor_names,
                        initial_guess_current_joints=False,  # Important to set it to False
                    ),
                ],
